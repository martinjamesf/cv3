...
const themes = [
  { name: "light", icon: sun, tooltip: "light theme" },
  { name: "dark", icon: moon, tooltip: "dark theme" },
  { name: "eighties", icon: eighties, tooltip: "80s theme" },
];

let currentThemeIndex = 0;

function applyTheme(index) {
  const { name, icon, tooltip } = themes[index];
  html.setAttribute("data-theme", name);
  switchTheme.innerHTML = icon;
  switchTheme.setAttribute("data-tooltip", tooltip);
  localStorage.setItem("themeIndex", index);
}

document.addEventListener("DOMContentLoaded", () => {
  const savedIndex = parseInt(localStorage.getItem("themeIndex"), 10);
  currentThemeIndex = isNaN(savedIndex) ? 0 : savedIndex;
